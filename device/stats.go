...
func (d *EdgeDevicePlugin) doStats(ctx context.Context, stats chan<- *device.StatsResponse, interval time.Duration) {
	defer close(stats)
...
func (d *EdgeDevicePlugin) writeStatsToChannel(stats chan<- *device.StatsResponse, timestamp time.Time) {
...
func (d *EdgeDevicePlugin) collectStats() ([]*deviceStats, error) {
	d.deviceLock.RLock()
